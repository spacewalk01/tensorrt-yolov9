...
class YoLov9TRT(object):
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        param:
            output:     A numpy likes [cx,cy,w,h,cls_id, cx,cy,w,h,conf,cls_id, ...] 
            origin_h:   height of original image
            origin_w:   width of original image
        return:
        ...
        confs = scores[np.arange(len(scores)), list(class_ids)].reshape(-1, 1)     
        class_ids = class_ids.reshape(-1, 1)   
        pred = np.concatenate((boxes, confs, class_ids), axis=1)
